...
/**
 * ✅ Solo IDs estables:
 * - Usa g.gameId (del feed) si existe
 * - Bloquea timestamps 13 dígitos
 * - NO usa g.id como fallback
 */
...
export default function NflTournamentPage() {
  const { user } = useAuth();
  ...
  const filtered = useMemo(() => {
    ...
    return games.filter((g: any) => {
      ...
      if (market !== "all") {
        // hide rows that don't have markets ready (but still show if you want)
        if (market === "spread") {
  ...
  async function handlePick(args: {
    ...
    // ✅ Evita combinar ML y Spread en mismo juego (igual que NBA)
          ...
          <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-2 text-white/60">
                Picks lock automatically at kickoff. Points update when games go{" "}
