...
export default function PaidTournamentDetailPage() {
  ...
  async function handleJoin() {
    ...
    try {
      ...
      const res = await fn({
        tournamentId: id,
        successUrl: `${origin}/tournaments/paid/${id}?entry=success`,
        cancelUrl:  `${origin}/tournaments/paid/${id}?entry=cancelled`,
      });
      if (res.data?.url) {
    ...
    return (
      <Protected>
        <div className="min-h-screen px-4 py-20 text-center">
          ...
          <div className="text-white/50">Torneo no encontrado</div>
          <Link href="/tournaments/paid" className="mt-4 inline-block text-sm text-blue-400">← Volver</Link>
        </div>
      </Protected>
    );
  }
      ...
      <div className="min-h-screen px-4 py-6">
        <div className="mx-auto max-w-2xl space-y-4">
          ...
          {entryResult === "success" && isPaid && (
            <div className="rounded-2xl border border-emerald-400/25 bg-emerald-500/8 px-5 py-4 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-white/40 mt-0.5">Tu entrada fue procesada. Ya estás participando en este torneo.</div>
              </div>
            </div>
          )}
          {entryResult === "cancelled" && (
            <div className="rounded-2xl border border-white/10 bg-white/4 px-5 py-4 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-white/30 mt-0.5">No se realizó ningún cargo. Puedes intentarlo de nuevo.</div>
              </div>
            </div>
          )}
          {isCancelled && (
            <div className="rounded-2xl border border-red-400/20 bg-red-500/6 px-5 py-4 flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-white/40 mt-0.5">No se alcanzó el mínimo de participantes. Todos los pagos fueron reembolsados automáticamente.</div>
              </div>
            </div>
          )}
          ...
          <div className="rounded-2xl border border-white/8 bg-[#0C0E14] overflow-hidden">
            ...
            <div className="px-5 py-5">
              ...
              <div className="mb-5">
                ...
                <div className="grid grid-cols-3 gap-2 mb-2">
                  {t.prizes.slice(0, 3).map((prize, i) => (
                    <div key={i} className={`rounded-xl border py-3 text-center ${
                      i === 0 ? "border-amber-400/30 bg-amber-400/10" :
                      i === 1 ? "border-slate-400/20 bg-slate-400/6" :
                                "border-orange-400/20 bg-orange-400/6"
                    }`}>
                      <div className="text-xl mb-0.5">{i === 0 ? "🥇" : i === 1 ? "🥈" : "🥉"}</div>
                      <div className={`text-lg font-black ${i === 0 ? "text-amber-300" : i === 1 ? "text-slate-300" : "text-orange-300"}`}>
                        {fmtUsd(prize)}
                      </div>
              ...
              <div className="grid grid-cols-3 gap-2 mb-5">
                {[
                  { label: "Participantes", val: String(t.participantCount), sub: `mín ${t.minPlayers}` },
                  { label: "Tiempo", val: timeLeft || "—", sub: fmtDate(t.deadline).split(",")[0] },
                  { label: "Pool mín.", val: fmtUsd(t.entryFee * t.minPlayers / 100 * 100), sub: "si se confirma" },
                ].map(({ label, val, sub }) => (
              ...
              <div className="mb-5 space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className={`font-bold ${minReached ? "text-emerald-400" : "text-amber-400/60"}`}>
                ...
                <div className="mb-4 rounded-xl border border-emerald-400/20 bg-emerald-500/8 px-4 py-3 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-white/35 mt-0.5">
                      {myEntry?.prizeRank
                        ? `🏆 Ganaste el #${myEntry.prizeRank} — ${fmtUsd(myEntry.prizeAmountCents ?? 0)} (${myEntry.prizeStatus === "pending_payout" ? "pago pendiente" : "pagado"})`
                        : "Haz tus picks cuando el torneo inicie."}
                    </div>
                  </div>
                </div>
              )}
              {isRefunded && (
                <div className="mb-4 rounded-xl border border-white/10 bg-white/4 px-4 py-3 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-white/30 mt-0.5">El torneo fue cancelado. Tu entrada fue reembolsada a tu tarjeta.</div>
                  </div>
                </div>
              )}
              ...
              {/* CTA */}
              {error && (
                <div className="mb-3 rounded-xl border border-red-400/20 bg-red-500/8 px-4 py-3 text-sm text-red-300">{error}</div>
              )}

              {canJoin && (
                <button onClick={handleJoin} disabled={joining}
                  className="w-full rounded-xl border border-amber-400/30 bg-amber-400/12 py-4 text-sm font-black text-amber-300 hover:bg-amber-400/18 active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
              ...
              {!isOpen && !isPaid && !isPending && !isCancelled && !isRefunded && (
                <div className="w-full rounded-xl border border-white/8 bg-white/[0.02] py-4 text-center text-sm text-white/30">
                  Registro cerrado
                </div>
              )}
              ...
              {canJoin && (
                <p className="mt-2.5 text-center text-[11px] text-white/20">
                  Pago seguro con Stripe · Si no se alcanza el mínimo de {t.minPlayers} jugadores tu entrada se reembolsa al 100%
                </p>
              )}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-white/6 bg-white/[0.015] px-5 py-4">
            <div className="text-[10px] font-bold uppercase tracking-wider text-white/25 mb-3">Reglas del torneo</div>
            <div className="space-y-2 text-xs text-white/40 leading-relaxed">
              ...
              <p>• Si el torneo no alcanza <span className="text-white/60 font-semibold">{t.minPlayers} participantes pagos</span> antes del cierre, se cancela y se reembolsa el 100%.</p>
              <p>• Los premios se distribuyen en efectivo al concluir el torneo. Stat2Win se reserva el derecho de solicitar verificación de identidad para pagos mayores a $600 (requerimiento IRS).</p>
