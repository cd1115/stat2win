...
type LeaderboardCallableResponse = {
  ok: boolean;
  rows?: LeaderboardCallableRow[];
};
...
function toLeaderRow(d: LeaderboardCallableRow, id: string): LeaderRow {
  ...
  return {
    ...
    pointsSpread:
      typeof d.pointsSpread === "number" ? d.pointsSpread : undefined,
    winsSpread: typeof d.winsSpread === "number" ? d.winsSpread : undefined,
    lossesSpread:
      typeof d.lossesSpread === "number" ? d.lossesSpread : undefined,
    pushesSpread:
      typeof d.pushesSpread === "number" ? d.pushesSpread : undefined,
    picksSpread:
      typeof d.picksSpread === "number" ? d.picksSpread : undefined,
...
export default function LeaderboardPage() {
 ...
 const getDisplayName = (r: LeaderRow) =>
  (r.username || r.displayName || "User").trim() || "User";
    ...
    async function load() {
      ...
      try {
        ...
        const callable = httpsCallable<
          { weekId: string; sport: Sport; market: Market },
          LeaderboardCallableResponse
        >(functions, "getLeaderboardWeek");
        ...
        const list: LeaderRow[] = rawRows.map((d, index) =>
          toLeaderRow(
            d,
            d.id ??
              d.uid ??
              `${SPORT}-${weekId}-${market}-${index}`
          )
        );

        list.sort((a: LeaderRow, b: LeaderRow) => {
          return (
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6 md:p-8">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <Link
            ...
            className="rounded-xl border border-blue-400/40 bg-blue-500/10 px-4 py-2 text-sm text-blue-100"
          >
            NBA
          </Link>
          ...
          {(["ALL", "ML", "SPREAD", "OU"] as Market[]).map((m) => {
            ...
            return (
              <button
                ...
                disabled={disabled}
                className={[
                  "rounded-xl border px-4 py-2 text-sm transition disabled:cursor-not-allowed disabled:opacity-40",
                  on
        ...
        <div className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-4">
          <div className="rounded-2xl border border-white/10 bg-black/25 p-4 shadow-[0_0_0_1px_rgba(255,255,255,.06),0_0_40px_rgba(99,102,241,.10)]">
            <div className="text-xs text-white/60">Players</div>
            <div className="mt-1 text-2xl font-semibold">{rows.length}</div>
            <div className="mt-1 text-xs text-white/50">Active this week</div>
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
            ...
            <div className="mt-1 text-2xl font-semibold">
              {myRank ? (
                <span className="inline-flex items-center gap-2">
                  ...
                  <span className="text-white/70">/ {rows.length}</span>
                </span>
              ) : (
            ...
            <div className="mt-1 text-xs text-white/50">
              {myRank ? "Keep climbing" : "Not ranked yet"}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
            <div className="text-xs text-white/60">Your record</div>
            <div className="mt-1 text-2xl font-semibold">
              {myRow ? (
            ...
            <div className="mt-1 text-xs text-white/50">
              {myRow ? `Picks: ${myRec.picks ?? myRow.picks ?? 0}` : "—"}
            </div>
          </div>
        </div>
          ...
          {!user?.uid ? (
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
              ...
              <div className="mt-1 text-white/60">
                Points appear after resolved picks exist for this week.
              </div>
              <div className="mt-4 flex gap-2">
                <Link
                  href={SPORT === "NBA" ? "/tournaments/nba" : "/tournaments/mlb"}
                    ...
                    >
                      ...
                      <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0">
                         ...
                         @{(r.username ?? r.displayName ?? "user").toLowerCase()}
                          </div>
                        </div>
                      </div>

                      <div className="text-right font-semibold">
                        {pts} <span className="font-normal text-white/50">pts</span>
                      </div>
